...
const char* lookup(const char* path)
{
    ...
    //указатель на последнее значение точки в part( Получается .gif)
    char *pathext = strrchr(path, '.');
   
    //Если значение из массива совпадает с .gif, то возвращаем image/gif. Если совпадений нет, то возвразаем NUUL.
    for (int i = 0; i < 8; i++)
        if (strcmp(ext[i], pathext) == 0) //Сравниваем значения в строках
            return my[i];
